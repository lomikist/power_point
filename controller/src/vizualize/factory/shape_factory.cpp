...
{
    register_shape("elipse", [](const model::Item& item)
    {
        return std::make_shared<core::CircleShape>(item);
    });
    register_shape("rect", [](const model::Item& item)
    {
        return std::make_shared<core::RectShape>(item);
    });
    register_shape("textbox", [](const model::Item& item)
    {
...
std::shared_ptr<Ishape> ShapeFactory::create_shape(const std::string& name, const model::Item& item)
{
